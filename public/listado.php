...
require __DIR__ . '/../src/Operaciones.php';
...
function conseguirNombres($id)
{
    $response = jaxon()->newResponse();

    $operacion = new Operaciones();

    $personas = $operacion->getDatosProd($id);
...
<body style="background: gray">
        ...
        <table class="table table-striped table-dark mt-3">
            ...
            <tbody>
                ...
                while ($filas = $stmt->fetch(PDO::FETCH_OBJ)) {
                    ...
                    echo "<td>";
                    echo<<<FIN
                    <button type='button' onclick='jaxon_conseguirNombres({$filas->id})'>
                        {$filas->nombre}</button>
                        , Precio: {$filas->pvp} (€)
    ...
    <?
    echo $jaxon->getCss(), $jaxon->getJs(), $jaxon->getScript();
